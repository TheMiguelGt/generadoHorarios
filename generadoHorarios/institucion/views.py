...
def TimeTableView(request,id):
    try:
        ...
        docema = DocenteMateria.objects.select_related('materia','docente','clase').filter(clase_id=section.id).order_by('start_time')
        admin = Admin.objects.all()
        coordina = Coordina.objects.all()
        docente = Docente.objects.all()
        pages = Page.objects.all()
        ...
        context_1 = {'section':section,'docema':docema,'time':time,'time_slot':time_slot,'admin':admin,'coordina':coordina,'docente':docente,'pages':pages,'disponi':disponi,'matedo':matedo}
...
def horario_render_pdf_view(request,*args, **kwargs):
    ...
    docema = DocenteMateria.objects.select_related('materia','docente','clase').filter(clase_id=horario.id).order_by('start_time')
